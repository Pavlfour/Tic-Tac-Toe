...
{
    char gameStatus=0;
    char str[]="---------"; //This string is the main table
    printTable(str);
    while (!gameStatus)
    {
        playerMove(str);
...
void printTable(char *str) //with two nested for loops we're displaying the inserted string in a table format
{
    int j=0;
    int step=3;
    cout<<"========="<<endl;
...
int checkStep(char *str)
{
    for(int i=0,j=0;j<3;j++,i+=3)
    {
        if(str[i]==str[i+1]&str[i]==str[i+2]&str[i]!='-') //this line checks the rows of the table
            return str[i];
        else if(str[j]==str[j+3]&str[j]==str[j+6]&str[j]!='-') //this for the columns aswell
            return str[j];
    }
    for(int i=0;i<2;i++)
    {
    ...
    for(int i=0;i<9;i++)
    {
        if(str[i]=='-') //if there's still empty space in the table, the game goes on
            return 0;
    }
